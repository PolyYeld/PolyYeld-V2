...
contract YELDV2 is ERC20('PolyYeld V2', 'YELD V2') {
...
// MasterChef is the master of Yeld. He can make Yeld and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once Yeld is sufficiently
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        //   pending reward = (user.amount * pool.accYeldPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accYeldPerShare` (and `lastRewardBlock`) gets updated.
    ...
    uint256 constant MAX_YELD_PERBLOCK= 0.1 ether;
    ...
    // The block number when Yeld mining starts.
    uint256 public startBlock;

    // Yeld referral contract address.
    ...
    // View function to see pending Yeldes on frontend.
    function pendingYeld(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accYeldPerShare = pool.accYeldPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0 && totalAllocPoint>0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 YeldReward = multiplier.mul(YeldPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accYeldPerShare = accYeldPerShare.add(YeldReward.mul(1e18).div(lpSupply));
        }
        return user.amount.mul(accYeldPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 YeldReward = multiplier.mul(YeldPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        Yeld.mint(devAddress, YeldReward.div(10));
        Yeld.mint(address(this), YeldReward);
        pool.accYeldPerShare = pool.accYeldPerShare.add(YeldReward.mul(1e18).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for Yeld allocation.
    function deposit(uint256 _pid, uint256 _amount, address _referrer) public nonReentrant {
        ...
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accYeldPerShare).div(1e18).sub(user.rewardDebt);
            if (pending > 0) {
                safeYeldTransfer(msg.sender, pending);
    ...
    // Safe Yeld transfer function, just in case if rounding error causes pool to not have enough FOXs.
    function safeYeldTransfer(address _to, uint256 _amount) internal {
        uint256 YeldBal = Yeld.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > YeldBal) {
            transferSuccess = Yeld.transfer(_to, YeldBal);
        } else {
            transferSuccess = Yeld.transfer(_to, _amount);
        }
        require(transferSuccess, "safeYeldTransfer: Transfer failed");
    }
    ...
    function updateEmissionRate(uint256 _YeldPerBlock) external onlyOwner {
        require(_YeldPerBlock<=MAX_YELD_PERBLOCK,"Too high");
        massUpdatePools();
        YeldPerBlock = _YeldPerBlock;
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        if (address(referral) != address(0) && referralCommissionRate > 0) {
            ...
            if (referrer != address(0) && commissionAmount > 0) {
                Yeld.mint(referrer, commissionAmount);
